angular.module('asti.application').factory('userService', ['$http', '$q', '$cookies', 'apiService',
    ...
    var username = null;
    var name = $cookies.get('asti.nameOperator');
    console.log(name);
	var encodedCredential = $cookies.get('asti.operator');
	...
	var logout = function(){
		var log = $q.defer()
		$cookies.remove('asti.operator');
		$cookies.remove('asti.nameOperator');
	...
	var login = function(email, password){
		...
		apiService.validateCredential(credential).then(
				function(data){
					...
					$cookies.put('asti.operator',encodedCredential);
					$http.defaults.headers.common['Authorization'] = 'xBasic ' + encodedCredential;
					log.resolve();
				},
				function(reason){
	...
	var getNameFromUsername = function(username){
		...
		$http.get('/api/v1/name?username='+u).then(
				function(response){
					name = response.data;
					$cookies.put('asti.nameOperator',name);
					p.resolve(name);
				},
				function(reason){
					p.reject(reason);
